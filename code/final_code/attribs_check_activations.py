...
n_groups = 4
expt_num = sys.argv[1]
PATH = f'../../data/models/no_mask_stanford_running_loss_VGG_pool_reproduce_sampled_accuracy.pt'
...
class thyroidActualDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {"image": im.float(), "comp": labels[1], "echo": labels[2], "margin": labels[3], "calc": labels[4], "type": torch.unsqueeze(labels[0], 0), "filename": name}
        return sample

'''
def save_activations(image_name, weights, features, linear_features):
    print(image_name)
    ...
    cv2.imwrite(image_name[0].replace('images','activation_attribs'), output_img)
'''
...
def save_activations(image_name, weights, features, linear_features, pool=True):
    ...
    cv2.imwrite(image_name[0].replace('images','activation_pool_attribs'), output_img)
...
def perform_test(model, dataset, display=False):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in tqdm(enumerate(test_generator), total=totiter):
            ...
            #print(x_test)
            pred_comp, pred_echo, pred_margin, pred_calc, features, linear_features = model(x_test)
